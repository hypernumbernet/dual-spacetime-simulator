...
pub struct VulkanBase {
    ...
    // not per frame-in-flight: a presentation may still be using a semaphore when the same
    // frame slot comes around, so reusing it triggers VUID-vkQueueSubmit-pSignalSemaphores-00067.
...
impl VulkanBase {
    ...
    pub fn new(
        ...
        let (image_available, in_flight) = create_frame_sync_objects(&device);
        let render_finished = create_image_semaphores(&device, images.len());
    ...
    pub fn recreate_swapchain(&mut self, window: &Window) {
        ...
        self.swapchain = sc;
        self.swapchain_image_views = create_image_views(&self.device, &images, format);

        // Recreate the per-image render-finished semaphores if the image count changed.
        if images.len() != self.render_finished_semaphores.len() {
            ...
            self.render_finished_semaphores = create_image_semaphores(&self.device, images.len());
        }
...
/// Creates per-frame-in-flight image-available semaphores and in-flight fences.
fn create_frame_sync_objects(device: &Device) -> (Vec<vk::Semaphore>, Vec<vk::Fence>) {
...
fn create_image_semaphores(device: &Device, count: usize) -> Vec<vk::Semaphore> {
    let sem_ci = vk::SemaphoreCreateInfo::default();
    (0..count)
        .map(|_| unsafe { device.create_semaphore(&sem_ci, None) }.unwrap())
        .collect()
}
