...
行います（共通の time-to-go で両脚を同期）。成功条件はターゲット正方形パッド内への着陸
（半辺 30 m）で、中心への精密寄せは求めません。接地後は直立スナップ（位置 PD 停止・
姿勢ゲイン強化）で素早く安定させます。CoM 高度がおおよそ 500 m 未満のままでは最終降下に
入らず、ゲートを超えてからソフト着陸に切り替わります（L とその場着陸と相互排他）。
