...
で減速し、ターミナル settle（Brake → Upright → Trim）で姿勢・横速度を静かにしてから
最終降下（Descend）にハンドオフします。降下の後半（低高度）は位置微調整をせず
直立＋ソフト接地にコミットします。**着陸成功**は内側 Chebyshev 箱（半辺 **12 m**、
`TARGET_SUCCESS_HALF_M`）。**描画パッド**は半辺 30 m（`TARGET_PAD_HALF_M`）。
Descend へのアームは CoM **≥480 m**（`GATE_ALT_MIN`）かつパッド上空で横速度・姿勢が
静かになったときの **離散 AND ゲート**（L とその場着陸と相互排他）。
