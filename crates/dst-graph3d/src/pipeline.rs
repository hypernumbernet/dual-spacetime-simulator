...
impl ParticleRenderPipeline {
    /// Creates graphics pipelines and persistent rendering resources for the graph viewer.
    pub fn new(base: &VulkanBase) -> Self {
        ...
        let display_buffer = DisplayParticleBuffer::new(
            device.clone(),
            Arc::clone(&allocator),
    ...
    fn draw_particles(
        ...
        let draw_count = self.display_buffer.particle_count();
        if draw_count == 0 {
            return;
        }
        ...
        unsafe {
            ...
            self.device.cmd_bind_descriptor_sets(
                ...
                0,
                &[self.display_buffer.descriptor_set()],
                &[],
            );
            self.device.cmd_push_constants(
                cb,
